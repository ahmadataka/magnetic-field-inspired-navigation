...
ARTIFACTS = benchmark_artifact_dir(ROOT, "dynamic_alpha_sweep_2d")
RAW_CSV = ARTIFACTS / "dynamic_alpha_sweep_raw.csv"
SUMMARY_CSV = ARTIFACTS / "dynamic_alpha_sweep_summary.csv"
SUMMARY_MD = ARTIFACTS / "dynamic_alpha_sweep_summary.md"
PLOT_PNG = ARTIFACTS / "dynamic_alpha_sweep.png"
...
SEEDS = list(range(5))
C_PERP_SCALE_FACTORS = [0.5, 0.75, 1.0, 1.25, 1.5, 2.0]
...
def _configs_for_scale(scale: float):
    base_pd = make_paper_pd_config()
    base_geom = make_paper_geometric_config()
    c_perp_value = base_pd.c_perp * scale
    config_pd = replace(base_pd, c_perp=c_perp_value)
    config_geometric = replace(base_geom, c_perp=c_perp_value)
    alpha_eff = c_perp_value / (MASS_REF * base_pd.speed_limit * base_pd.speed_limit)
    return config_pd, config_geometric, c_perp_value, alpha_eff
...
def _summarize(raw_rows: list[dict[str, str | float]]) -> list[dict[str, str | float]]:
    ...
    for row in raw_rows:
        grouped.setdefault((str(row["scenario"]), str(row["method"]), float(row["alpha_eff"])), []).append(row)

    summary_rows: list[dict[str, str | float]] = []
    for scenario_name in SCENARIO_NAMES:
        for method_name in METHOD_SPECS:
            for scale in C_PERP_SCALE_FACTORS:
                _, _, c_perp_value, alpha_eff = _configs_for_scale(scale)
                rows = grouped[(scenario_name, method_name, alpha_eff)]
...
def _plot(summary_rows: list[dict[str, str | float]]) -> None:
    metrics = [
        ("success_rate_mean", None, r"success rate"),
        ("min_clearance_mean", "min_clearance_std", r"min clearance [m]"),
        ("final_goal_distance_mean", "final_goal_distance_std", r"final goal distance [m]"),
    ]
    ...
    for row_idx, scenario_name in enumerate(SCENARIO_NAMES):
        ...
        for col_idx, (mean_key, std_key, label) in enumerate(metrics):
            ...
            for method_name, spec in METHOD_SPECS.items():
                ...
                rows.sort(key=lambda item: float(item["alpha_eff"]))
                x = [float(row["alpha_eff"]) for row in rows]
            ...
            ax.set_xlabel(r"$\alpha_{\mathrm{eff}} = c_{\perp} / (m v_{\mathrm{ref}}^2)$")
            ax.set_ylabel(label)
    ...
    fig.suptitle(r"Dynamic 2D alpha sensitivity sweep with fixed $v_{\mathrm{ref}}$", y=0.995)
...
def _write_markdown(summary_rows: list[dict[str, str | float]]) -> None:
    lines = [
        ...
        "",
        r"`alpha_eff = c_perp / (m v_ref^2)`",
        "",
        f"- scenarios: `{SCENARIO_NAMES}`",
        f"- seeds: `{SEEDS}`",
        f"- c_perp scale factors: `{C_PERP_SCALE_FACTORS}`",
        f"- reference mass: `{MASS_REF}`",
        "",
    ...
    for row in summary_rows:
        ...
        lines.append(
            f"| {row['scenario']} | {row['method_label']} | {float(row['c_perp']):.3f} | {float(row['alpha_eff']):.3f} | {float(row['success_rate_mean']):.3f} | {float(row['collision_rate_mean']):.3f} | {float(row['safety_violation_rate_mean']):.3f} | {_format(float(row['final_goal_distance_mean']), float(row['final_goal_distance_std']))} | {_format(float(row['min_clearance_mean']), float(row['min_clearance_std']))} | {float(row['worst_min_clearance']):.3f} | {time_text} |"
        )
...
def main() -> None:
    ...
    for scenario_name in SCENARIO_NAMES:
        ...
        for scale in C_PERP_SCALE_FACTORS:
            config_pd, config_geometric, c_perp_value, alpha_eff = _configs_for_scale(scale)
            for seed in SEEDS:
                scenario, initial_velocity = _perturb_scenario(base_scenario, seed)
    ...
    print(f"dynamic_alpha_raw={RAW_CSV}")
    print(f"dynamic_alpha_summary_csv={SUMMARY_CSV}")
    print(f"dynamic_alpha_summary_md={SUMMARY_MD}")
    print(f"dynamic_alpha_plot={PLOT_PNG}")
