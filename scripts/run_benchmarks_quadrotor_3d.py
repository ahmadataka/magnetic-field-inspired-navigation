...
def _plot_projection(ax: plt.Axes, scenario, histories: dict[str, list[dict[str, float]]], axes: tuple[str, str]) -> None:
    ...
    for obstacle in scenario.obstacles.obstacles:
        if isinstance(obstacle, SphereObstacle):
            ax.scatter(
                obstacle.center[axis_map[x_key]],
                obstacle.center[axis_map[y_key]],
                color="#d62728",
                s=80,
                alpha=0.35,
            )
    ...
    ax.scatter(scenario.start[axis_map[x_key]], scenario.start[axis_map[y_key]], color="#0f766e", s=60, marker="o")
    ax.scatter(scenario.goal[axis_map[x_key]], scenario.goal[axis_map[y_key]], color="#111111", s=90, marker="*")
    ax.set_xlabel(x_key)
    ax.set_ylabel(y_key)
...
def _build_interactive_html(scenarios_data: list[dict[str, object]]) -> str:
    html_parts = [
        ...
        "  <title>Quadrotor 3D Benchmark Comparison</title>",
        ...
        "<body>",
        "  <main>",
        "    <h1>Interactive Quadrotor 3D Benchmark Comparison</h1>",
        "    <p>Rotate, pan, and zoom each scene. Use the buttons to show one or more algorithms. The view auto-zooms to the currently visible trajectories and obstacle geometry.</p>",
    ]
    ...
    script_lines = [
        ...
        "  applyAlgorithmSelection(plotId, traces, selectedAlgorithmsForPlot(plotId));",
        "}",
    ]
...
def main() -> None:
    ...
    hover_thrust = config_pd.quadrotor_mass * config_pd.quadrotor_gravity

    artifacts = benchmark_artifact_dir(ROOT, "quadrotor_3d")
    ...
    for row_axes, scenario in zip(axes, scenarios):
        histories = {
            "paper_pd_3d": simulate_quadrotor(
                _initial_state(scenario.start, hover_thrust),
                scenario.goal,
                scenario.obstacles,
                config_pd,
                navigator=MagneticFieldNavigator3D(config_pd),
            ),
            "paper_geometric_3d": simulate_quadrotor(
                _initial_state(scenario.start, hover_thrust),
                scenario.goal,
                scenario.obstacles,
                config_geometric,
            ...
            "haddadin_3d": simulate_quadrotor(
                _initial_state(scenario.start, hover_thrust),
                scenario.goal,
                scenario.obstacles,
                config_pd,
            ...
            "sabattini_3d": simulate_quadrotor(
                _initial_state(scenario.start, hover_thrust),
                scenario.goal,
                scenario.obstacles,
                config_pd,
    ...
    fig.tight_layout()
    plot_path = artifacts / "benchmark_comparison_quadrotor_3d.png"
    fig.savefig(plot_path, dpi=180)
    plt.close(fig)

    interactive_path = artifacts / "benchmark_comparison_quadrotor_3d.html"
    interactive_path.write_text(_build_interactive_html(interactive_scenarios), encoding="utf-8")

    csv_path = artifacts / "benchmark_metrics_quadrotor_3d.csv"
    with csv_path.open("w", newline="") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(summary_rows[0].keys()))
        writer.writeheader()
        writer.writerows(summary_rows)

    print(f"benchmark_plot_quadrotor_3d={plot_path}")
    print(f"benchmark_plot_quadrotor_3d_interactive={interactive_path}")
    print(f"benchmark_metrics_quadrotor_3d={csv_path}")
