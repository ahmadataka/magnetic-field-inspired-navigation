...
from mfinav.utils.paths import generated_world_dir, smoke_artifact_dir  # noqa: E402


ARTIFACTS = smoke_artifact_dir(ROOT, "webots_crazyflie_dynamic")
WORLD_DIR = generated_world_dir(ROOT, "crazyflie", "dynamic_smoke")
WORLD_PATH = WORLD_DIR / "moving_sphere_crossing_3d.wbt"
...
def _ensure_generated_project_links() -> None:
    ...
    controllers_link = WORLD_DIR.parent / "controllers"
...
def _transform_scenario_for_webots(scenario):
    ...
    for obstacle in scenario.obstacles.obstacles:
        if isinstance(obstacle, MovingSphereObstacle):
            ...
            raise ValueError(f"Unsupported obstacle type in dynamic Crazyflie smoke transform: {type(obstacle)!r}")
...
def _sphere_surface(center: np.ndarray, radius: float, num: int = 24) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    u = np.linspace(0.0, 2.0 * math.pi, num)
    v = np.linspace(0.0, math.pi, num)
...
def _plot(results: dict[str, dict[str, object]], scenario) -> Path:
    ...
    key_index = {"x": 0, "y": 1, "z": 2}

    for ax, (a, b) in zip(axes, projections):
        ia = key_index[a]
        ib = key_index[b]
        for method_name, result in results.items():
            ...
            ax.scatter(history[-1][a], history[-1][b], color=METHOD_SPECS[method_name]["color"], marker="x", s=50)

        start = scenario.start
        goal = scenario.goal
        ax.scatter([start[ia]], [start[ib]], color="#16a34a", s=55, marker="o", label="start")
        ax.scatter([goal[ia]], [goal[ib]], color="#111111", s=90, marker="*", label="goal")
        ...
        for obstacle in scenario.obstacles.obstacles:
            ...
            if start_snapshot["kind"] == "sphere":
                if {a, b} <= {"x", "y"}:
                    circle = plt.Circle(
                        (start_snapshot["center"][ia], start_snapshot["center"][ib]),
                        start_snapshot["radius"],
                        color="#ef4444",
                        alpha=0.12,
                    )
                    ax.add_patch(circle)
                    circle_end = plt.Circle(
                        (end_snapshot["center"][ia], end_snapshot["center"][ib]),
                        end_snapshot["radius"],
                        color="#f59e0b",
                        alpha=0.10,
                    )
                    ...
                    ax.scatter(
                        [start_snapshot["center"][ia]],
                        [start_snapshot["center"][ib]],
                        color="#ef4444",
                        s=80,
                        alpha=0.55,
                    )
                    ax.scatter(
                        [end_snapshot["center"][ia]],
                        [end_snapshot["center"][ib]],
                        color="#f59e0b",
                        s=80,
                        alpha=0.55,
                    )
    ...
    fig.suptitle(f"Webots Crazyflie dynamic smoke test: {scenario.name}")
    fig.tight_layout(rect=(0, 0, 1, 0.93))
    plot_path = ARTIFACTS / "webots_crazyflie_dynamic_smoke.png"
...
WorldInfo {{
  title "Webots Crazyflie dynamic smoke"
  basicTimeStep 32
}}
Viewpoint {{
...
def _run_method(scenario, scale: float, method_name: str) -> dict[str, object]:
    history_path = ARTIFACTS / f"history_{method_name}.csv"
    summary_path = ARTIFACTS / f"summary_{method_name}.json"
    console_path = ARTIFACTS / f"console_{method_name}.log"
    ...
    if not history_path.exists():
        raise RuntimeError(f"Webots Crazyflie dynamic smoke test did not produce a history for {method_name}. See {console_path}.")
    ...
    return {"history": history, "summary": summary, "console": console_path}
...
def main() -> None:
    ...
    scenario = make_dynamic_scenarios_3d()[0]
    transformed_scenario, scale = _transform_scenario_for_webots(scenario)
    results: dict[str, dict[str, object]] = {}

    for method_name in METHOD_SPECS:
        results[method_name] = _run_method(transformed_scenario, scale, method_name)

    plot_path = _plot(results, transformed_scenario)
    summary_rows = []
    for method_name, result in results.items():
        ...
        summary_rows.append(
            {
                "scenario": transformed_scenario.name,
                "method": method_name,
    ...
    metrics_path = ARTIFACTS / "webots_crazyflie_dynamic_metrics.csv"
    ...
    for row in summary_rows:
        print(
            f"{row['method']}: status={row['status']} success={int(row['success'])} "
            f"collision={int(row['collision'])} final_goal_distance={row['final_goal_distance']:.3f} "
            f"min_clearance={row['min_clearance']:.3f}"
        )
    print(f"webots_crazyflie_dynamic_plot={plot_path}")
    print(f"webots_crazyflie_dynamic_metrics={metrics_path}")
