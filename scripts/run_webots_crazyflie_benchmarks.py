...
def _rescale_scenario_for_webots(scenario):
    ...
    scaled_obstacles = [PolygonObstacle(vertices=np.asarray(obstacle.vertices, dtype=float) * scale) for obstacle in scenario.obstacles.obstacles]
    return replace(
        scenario,
        ...
        description=f"{scenario.description} Rescaled by {scale:.3f} for Webots Crazyflie arena.",
    )
...
def _scenario_to_payload(scenario) -> dict[str, object]:
    return {
        ...
        "start": [float(scenario.start[0]), float(scenario.start[1]), START_HEIGHT],
        "goal": [float(scenario.goal[0]), float(scenario.goal[1]), HOVER_HEIGHT],
...
def _triangulate_polygon(vertices: np.ndarray) -> list[tuple[int, int, int]]:
    ...
    while len(remaining) > 3 and guard < 1000:
        ...
        for idx in range(len(remaining)):
            ...
            break
        if not ear_found:
            raise ValueError("Failed to triangulate polygon for Webots Crazyflie world generation.")
    if len(remaining) == 3:
        triangles.append((remaining[0], remaining[1], remaining[2]))
    return triangles


def _obstacle_shape_block(vertices: np.ndarray, height: float = 0.42) -> str:
...
def _world_text(scenario) -> str:
    ...
    size_x = x_max - x_min
    size_y = y_max - y_min
    viewpoint_x = center_x - 0.6 * size_x
    viewpoint_y = center_y - 0.45 * size_y
    viewpoint_z = 2.3
...
WorldInfo {{
  title "MFI Webots Crazyflie benchmark - {scenario.name}"
  basicTimeStep 32
}}
Viewpoint {{
  orientation -0.16377740306514843 0.3701985956311306 0.9143043323986831 0.653664866547213
...
Floor {{
  size {max(size_x, size_y, 3.0):.6f} {max(size_x, size_y, 3.0):.6f}
}}
DEF GOAL Pose {{
  translation {float(scenario.goal[0]):.6f} {float(scenario.goal[1]):.6f} 0.03
  children [
    Shape {{
      appearance PBRAppearance {{
        baseColor 0 0.8 0.2
...
{obstacles_text}
Crazyflie {{
  translation {float(scenario.start[0]):.6f} {float(scenario.start[1]):.6f} {START_HEIGHT:.6f}
  rotation 0 0 1 0
  name "Crazyflie"
...
def _plot_scenario(ax: plt.Axes, scenario, histories: dict[str, list[dict[str, float]]]) -> None:
    ...
    ax.set_title(f"{scenario.name.replace('_', ' ')} (webots crazyflie)")
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ax.axis("equal")
...
def _run_one_scenario(scenario, method_name: str) -> tuple[list[dict[str, float]], dict[str, object], Path]:
    ...
    if not history_path.exists():
        raise RuntimeError(f"Webots Crazyflie did not produce a history for scenario {scenario.name} ({method_name}). See {console_path}.")
...
def main() -> None:
    ...
    print(f"benchmark_plot_webots_crazyflie={PLOT_PNG}")
    print(f"benchmark_metrics_webots_crazyflie={METRICS_CSV}")
