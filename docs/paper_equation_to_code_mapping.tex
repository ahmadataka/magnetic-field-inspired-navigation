...
\hline
Magnetic boundary-following field derived from that artificial current & lines 355--357 & Approximate & The paper-mode implementation is now cross-product based and much closer to the ROS/paper construction, but it is still not verified as the exact paper equation. \\
\hline
Property that \(F_b\) does not affect speed & lines 355--357, 599--600 & Approximate / violated by simulation assumptions & The paper-mode field is closer to the intended orthogonal construction, but the simulator still applies acceleration and speed clipping. \\
\hline
\end{longtable}
...
\hline
Exact paper field expression for \(F_a\) & lines 372--375 & Approximate & The implementation is now a closer hard-threshold projected repulsive term, but it is still not confirmed as a line-by-line reconstruction of the paper equation. \\
\hline
Property that \(F_a\) does not affect speed & lines 372--375 & Approximate & The paper-mode projection is explicitly intended to preserve this property, but the full claim is still entangled with our simulator assumptions. \\
\hline
\end{longtable}
...
\begin{enumerate}[leftmargin=*]
  ...
  The paper-mode implementation is now much closer, but we still have not
  verified it line by line against the 2022 paper’s exact field equation and
  3D derivation.

  \item \textbf{Collision-avoidance field \(F_a\)}.
  The paper-mode implementation is now closer and explicitly projected, but it
  is still not confirmed as the exact paper expression.
...
\hline
Boundary-following magnetic field \(F_b\) & Approximate & Current paper mode is much closer, but the exact field law is still not fully verified. \\
\hline
Collision-avoidance field \(F_a\) & Approximate & Closer than before, but still not fully verified equation-by-equation. \\
\hline
